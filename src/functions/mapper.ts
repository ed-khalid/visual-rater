...
export const mapSongToRaterUIItem = (song: Song, album: Album, artist:Artist): RatedSongUIItem   => ({id: song.id, name: song.name, score:song.score!, thumbnail:album.thumbnail!, tier: 1, overlay:album.dominantColor!, number:song.number, nodeRef:createRef(), artistId: artist.id, albumId: album.id, albumName:album.name, artistName:artist.name });
export const mapAlbumToRaterUIItem = (album: Album): RaterUIItem => ({ id: album.id, name: album.name , score:album.score!, thumbnail:album.thumbnail!, tier: 1, nodeRef:createRef(), overlay:album.dominantColor!})
export const mapArtistToRaterUIItem = (artist: Artist): RaterUIItem => ({ id: artist.id, name: artist.name , score:artist.score!, thumbnail:artist.thumbnail!, tier:1,nodeRef:createRef(), overlay: artist.dominantColor!})


export const mapUIItemToRaterUIItem = (item:MusicUIItem, tier:number): RaterUIItem  => ({...item, tier, nodeRef: createRef()})   