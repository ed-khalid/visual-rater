...
export const mapSongToUIItem = (song: Song, album: { thumbnail?:Maybe<string>, thumbnailDominantColors?:Maybe<Array<string>>, name: string }, artist:{ name: string}): SongUIItem   => ({ id: song.id, artistId: song.artistId, albumId: song.albumId, name: song.name , score:song.score!, thumbnail:album.thumbnail!, overlay:album.thumbnailDominantColors!, number:song.number,albumName:album.name, artistName:artist.name});


export const mapSongToFatSong = (song:Song, album:{thumbnail?:Maybe<string>, thumbnailDominantColors?:Maybe<Array<string>>, name:string }, artist:{name: string, id: string} ): FatSong  => {
    return {
        song,
        album,
        artist
